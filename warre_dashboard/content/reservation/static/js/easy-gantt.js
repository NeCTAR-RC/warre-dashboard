...
$.fn.gantt = function (options) {
    ...
    // let firstDay = moment(dtStart).subtract(7, "days").format("DD/MM/YYYY"); // Get the date 7 days before the start date
    ...
    $.each(tasks, function(index, task) {
        ...
        let taskColor = (task.color)? task.color: '#39833e';
            ...
            $.map(data, function(val, i){
                ...
                if(val.id == id){
                    if(moment(val.date_start, "DD/MM/YYYY").isAfter(moment(lastDate, 'DD/MM/YYYY'))){
                        ...
                        let taskColorS = (val.color)? val.color: '#39833e';
